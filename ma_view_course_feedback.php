...
<div class="toping">
    <h2 style="margin: 0;">Course Feedbacks</h2>
</div>
<div class="row py-3">
            <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="test_Madashbord.php">Home</a></li>
                <li class="breadcrumb-item active" aria-current="page">Course Feedbacks</li>
            </ol>
            </nav>
</div>
...
<div class="content">
    ...
    <div class="filter-container">
        <div class="filter-item">
            <label for="filter1">Accedemic year</label>
        ...
        <div class="filter-item">
            <label for="filter2">Semester</label>
            <select id="filter2" name="filter2">
                <option value="">Select</option>
                <?php
                $query = "SELECT DISTINCT Current_semester FROM student";
                $stmt = $pdo->query($query);
                $selectedSemester = isset($_POST['filter2']) ? $_POST['filter2'] : '';

                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    $selected = ($row['Current_semester'] == $selectedSemester) ? 'selected' : '';
                    echo '<option value="' . $row['Current_semester'] . '" ' . $selected . '>' . $row['Current_semester'] . '</option>';
                }
                ?>
            </select>
        </div>
        ...
        <div class="filter-item">
            <label for="filter3">Course_code</label>
            <select id="filter3" name="filter3">
                <option value="">Select</option>
                <?php
                $query = "SELECT DISTINCT CourseCode FROM course";
                $stmt = $pdo->query($query);
                $selectedCourseCode = isset($_POST['filter3']) ? $_POST['filter3'] : '';
    ...
    <thead>
        <tr>
            <th>Reg No</th>
            <?php
            // Step 1: Retrieve Question IDs from the database
            $questionQuery = "SELECT QuestionID FROM question WHERE Type='C'";
    ...
    <tbody>
        <?php
        // Step 2: Retrieve students' feedback based on selected filters
        $whereClause = " WHERE 1=1 ";
        $parameters = array();

        if (!empty($_POST['filter1'])) {
            $whereClause .= " AND Batch = :badge ";
            $parameters[':badge'] = $_POST['filter1'];
        }

        if (!empty($_POST['filter2'])) {
            $whereClause .= " AND Current_semester = :semester ";
            $parameters[':semester'] = $_POST['filter2'];
        }

        if (!empty($_POST['filter3'])) {
            $whereClause .= " AND CourseID = :courseID ";
            $parameters[':courseID'] = $_POST['filter3'];
        }

        else{
            $whereClause .= " AND CourseID = :courseID ";
            $parameters[':courseID'] = "EC5080";
        }


        $feedbackQuery = "SELECT s.RegNo AS StudentId, GROUP_CONCAT(cf.Rating) AS Feedbacks 
                          FROM student s
                          INNER JOIN course_feedback cf ON s.RegNo = cf.StudentId
                          {$whereClause}
                          GROUP BY s.RegNo";
